...
A single Claude Code skill (`stock-trading`) plus its Python safety layer. There is no build system, no package manager, and no application server (there is a small unittest suite for `risk.py`). "Running the code" means invoking the skill inside a Claude Code session — the skill itself drives MCP tools (Alpaca + TradingView) and shells out to a few small stdlib Python scripts.

The canonical entry point is the skill file, not any Python module. **Read [.claude/skills/stock-trading/SKILL.md](.claude/skills/stock-trading/SKILL.md) before touching anything else** — it defines the trading phases, token-efficiency rules, exclusion logic, and the autonomous-experiment doctrine that the Python layer only partially enforces.

Once invoked, the skill runs end-to-end with autonomous trade authority — Claude Code decides, `risk.py` enforces hard limits, Alpaca executes, and the audit log records everything. There is no human checkpoint between the reasoning layer and a placed order. Future Claude sessions working in this repo should assume that every change to `SKILL.md` is a change to an autonomous agent's behavior, and every change to `risk.py` or `config.json`'s `risk` block is a change to the agent's entire safety envelope.
...
- **Paper trading is the default and the whole repo is tuned for it.** `ALPACA_PAPER=true` in `.env`. Several `config.json` values (`max_position_pct_of_equity`, `max_session_pct_of_cash`, `max_spread_pct_of_midpoint`) are deliberately loose for paper mode and documented with `_*_note` siblings that say what to tighten before going live. Do not edit those without reading the notes and the promotion checklist in SKILL.md. Under the autonomous doctrine, the tighter values documented in those notes are the correct target regardless of paper/live — the only thing paper mode buys is that a miscalibration doesn't cost real money, not a second pair of eyes. The position/session caps are queued for a follow-up tightening pass under a fresh `experiment_id`; the spread cap stays loose until the minimum-trade-rule / screen-filter interaction is reworked (see `_max_spread_pct_note`).
...
- **The minimum-trade rule is intentional.** Phase 4 must place at least one BUY per run unless it's a dry run, the account is blocked, every candidate failed its dossier fetch, or the "too-garbage-to-trade" waiver fires. A run of all HOLDs has to override the weakest HOLD into a BUY. This exists because the skill is an autonomous agent producing an experimental dataset: nobody is going to review a HOLD afterward and learn from it, so an all-HOLD run is a wasted observation. The risk layer, not the minimum-trade rule, is what prevents bad trades from becoming catastrophic. See the "Autonomous-experiment doctrine" in SKILL.md for the reasoning. Don't "fix" this by loosening it to "HOLD if signals are weak" — that would trade empty runs for the illusion of prudence.
